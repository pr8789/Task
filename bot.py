...
def db_add(uid: int, svc: str, issuer: str, enc_secret: str,
    ...
    Returns False if the (uid, svc) pair already exists — caller should
    offer the user a chance to update instead.
    Uses insert_one (not upsert) so we can distinguish new vs. existing.
    """
    try:
        ...
        # Always invalidate cache so the UI reflects the real DB state
        _invalidate_cache(uid)
        return False
